...
class PlayAreas(db.Model):
    ...
    pfctNm = db.Column(db.String(255), nullable=False, index=True)  # 시설 이름
    pfctSn = db.Column(db.String(50), nullable=False)  # 시설 고유 번호
    instlPlaceCdNm = db.Column(db.String(255), nullable=True,index=True)  # 설치 장소 코드 이름
    ...
    idrodrCd = db.Column(db.String(50), nullable=True, index=True)  # 실내/실외 여부 코드
